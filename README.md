...
```
Now run the analysis for count for cig, mar, and binge.
...
cigMissingTest = summary(cigMissing[[1]])
cigMissingTest$df.residual
...
for(i in 1:m){
  output[[i]] = hurdle(MJ30D ~   RSKMJ.1*R_WHITE_N + RSKMJ.1*AGE.1 + RSKMJ.1*REL_IMP.1 + RSKMJ.1*INCOME + RSKMJ.1*SEX_PR+ RSKMJ.1*GENDER , data = CCPEBaselineMissing[[i]], dist = "poisson", zero.dist = "binomial")
  output[[i]] = summary(output[[i]])
  coef_output[[i]] = output[[i]]$coefficients$count[,1]
  se_output[[i]] = output[[i]]$coefficients$count[,2]
}
...
for(i in 1:m){
  output[[i]] = hurdle(BINGE530D ~   RSKALC.1*R_WHITE_N + RSKALC.1*AGE.1 + RSKALC.1*REL_IMP.1 + RSKALC.1*INCOME + RSKALC.1*SEX_PR+ RSKALC.1*GENDER , data = CCPEBaselineMissing[[i]], dist = "poisson", zero.dist = "binomial")
  output[[i]] = summary(output[[i]])
  coef_output[[i]] = output[[i]]$coefficients$count[,1]
  se_output[[i]] = output[[i]]$coefficients$count[,2]
}
...
cigMissingTest = summary(cigMissing[[1]])
cigMissingTest$coefficients$count[,2]

for(i in 1:m){
  output[[i]] = hurdle(CIG30D ~   RSKCIG.1*R_WHITE_N + RSKCIG.1*AGE.1 + RSKCIG.1*REL_IMP.1 + RSKCIG.1*INCOME + RSKCIG.1*SEX_PR+ RSKCIG.1*GENDER , data = CCPEBaselineMissing[[i]], dist = "poisson", zero.dist = "binomial")
  output[[i]] = summary(output[[i]])
  coef_output[[i]] = output[[i]]$coefficients$zero[,1]
  se_output[[i]] = output[[i]]$coefficients$zero[,2]
}
